...
public class bookings {
    ...
    public boolean createBooking(int numberOfTicketsCreate, Date bookingTimeCreate, int idCreate, int cinemaNumberCreate, ArrayList<Cinema> cinemas,int sessionIndex) {
        ...
        if (getRowFree == -1)
            return false;
        startseat = cinemas.get(cinema).getSession().get(sessionIndex).getSessionRows().get(getRowFree).firstFreeSeatInRow();
        ...
        rowname = cinemas.get(cinema).getSession().get(sessionIndex).getSessionRows().get(rownumber).getRowname();
        id = idCreate;
        return true;
    }
    ...
    public boolean changeBooking(int cinemaNumbernew, Date bookingTimenew, int numberOfTicketsnew, ArrayList<Cinema> cinemas,int sessionIndex) {
        ...
        int getRowFree = cinemas.get(cinema).getSession().get(sessionIndex).availableRow(numberOfTicketsnew);
        if (getRowFree == -1)
            return false;

        //now free up our old booking which can be conviently done with the row + start seat of old booking
        cinemas.get(cinema).getSession().get(sessionIndex).freeSeats(numberOfTickets, getRowFree, startseat);
        //now that old bookings have been reverted and fixed up assign new booking
        cinemas.get(cinema).getSession().get(sessionIndex).bookSeats(numberOfTicketsnew, getRowFree);
    ...
    public boolean cancelBooking(ArrayList<Cinema> cinemas,int sessionIndex) {
        ...
        cinemas.get(cinemaNumber).getSession().get(sessionIndex).freeSeats(numberOfTickets, rownumber, startseat);
        return true;
    }

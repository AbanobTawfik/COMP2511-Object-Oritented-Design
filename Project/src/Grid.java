...
public class Grid {
    ...
    public void tryMoveVehicle(GridVehicle v) {
        ...
        if (v.getVehicle().isHorizontal()) {
            //moving to the right ->
            if (!left && (v.getCol() > 0 && v.getCol() < GridVariables.BOARD_SIZE  - v.getVehicle().getSize()) &&
            ...
            //moving up
            if (up && (v.getRow()> 0 && (v.getRow() < GridVariables.BOARD_SIZE - v.getVehicle().getSize()))&&
                    grid[v.getCol()][v.getRow() -1] == null) {
                //now we want to clear up where the car was
                for (int i = 0; i < v.getVehicle().getSize(); i++)
    ...
    public boolean equals(Object o) {
        ...
        Grid grid1 = (Grid) o;
        return getGoalRow() == grid1.getGoalRow() &&
                numberOfMoves == grid1.numberOfMoves &&
                getGridScore() == grid1.getGridScore() &&
                Arrays.equals(getGrid(), grid1.getGrid()) &&
                Objects.equals(getGridVehicles(), grid1.getGridVehicles());
    }

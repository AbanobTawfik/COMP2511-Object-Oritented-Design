...
public class ASearch implements Heuristic<searchNode>{
    ...
    public int getShipmentScore(searchNode d) {
        ...
        if(d.getChildren().size()>0) {
            for (int i = 0; i < schedule.size(); i++) {
                if (schedule.contains(new DirectedEdge(d.getChildren().getLast().getTo(), schedule.get(i).getTo()))) {
                    int temp = g.getEdges()[schedule.get(i).getTo().getIndexOnGraph()][d.getChildren().getLast().getTo().getIndexOnGraph()];
                    temp += d.getChildren().getLast().getTo().getRefuellingTime();
                    if (temp < minShipment) {
                        flag = true;
                        minShipment = temp;
                    }
        ...
        if(d.getChildren().size()  == 0){
            for (int i = 0; i < schedule.size(); i++) {
                if (schedule.contains(new DirectedEdge(d.getShipment().getTo(), schedule.get(i).getTo()))) {
                    int temp = g.getEdges()[schedule.get(i).getTo().getIndexOnGraph()][d.getShipment().getTo().getIndexOnGraph()];
                    temp += d.getShipment().getTo().getRefuellingTime();
                    if (temp < minShipment) {
                        flag = true;
                        minShipment = temp;
                    }

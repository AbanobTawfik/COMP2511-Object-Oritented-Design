...
public class ASearch implements Heuristic<searchNode> {
     ...
     *
     * @return a ArrayList of directed edges which is a series of shipments that can be linked into a path
     */
    ...
    /**
     * this method will create a path from a search node which is a ArrayList of directed edges that can
     ...
     * @return the ArrayList of directed edges which is a representation of the path used in the print function
     */
     ...
     * aka you are on sydney and you have for example sydney to (SOME RANDOM PORT), then the minimum distance to closest shipment is 0
     ...
     * in every situation for this heuristic h(x) is less than H*(x)
     ...
     * Sydney to (Vancouver to Manila, shanghai to Singapore) and <br>
     * Sydney to (Shanghai to Singapore, Vancouver to Manila) <br>
     ...
     */
    /*
     * this method will add the absolute weighting of all the remaining shipments to complete the schedule
     * to do this i scan through the schedule, and if the shipment already exists in the constructed path, skip over
     * otherwise, we will add that shipments, refuelling cost FROM and the edge weight of the shipment
     */
    public int getScheduleScoreRemaining(searchNode s) {
        //initalise sum as 0, if all the shipments are completed, this will return 0
        int sum = 0;
        //initalise a list that will contain the connected path
        ArrayList<DirectedEdge> d = new ArrayList<DirectedEdge>();
        //if there are more than 1 shipments in the path
        if (s.getChildren().size() > 0) {
                //scan through the entire searchnode's path
            for (int i = 0; i < s.getChildren().size() - 1; i++) {
                //add the current shipment to the list, first part
                d.add(new DirectedEdge(s.getChildren().get(i).getFrom(), s.getChildren().get(i).getTo()));
                //second part connection to next shipment, add the last node from the current shipment
                //to the first ndoe in the next shipment, to connect the path
                d.add(new DirectedEdge(s.getChildren().get(i).getTo(), s.getChildren().get(i + 1).getFrom()));
            }
            //since the loop above scans to the second last shipment only to avoid null errors
            //add the last shipment in as normal
            d.add(new DirectedEdge((s.getChildren().get(s.getChildren().size()-1).getFrom()), s.getChildren().get(s.getChildren().size()-1).getTo()));
        }
        //if there is only 1 shipment in the path
        if (s.getChildren().size() == 1) {
            //add that shipment in to the list only
            d.add(new DirectedEdge(s.getChildren().get(0).getFrom(), s.getChildren().get(0).getTo()));
        }
        //now we want to scan through the schedule
        for (int i = 0; i < schedule.size(); i++) {
            //if the path contains the current shipment in the schedule, we want to skip over it
            if (d.contains(schedule.get(i)))
                continue;
            //otherwise we want to sum the absolute weight of the shipment by
            //adding the refuelling time of the FROM port
            sum += schedule.get(i).getFrom().getRefuellingTime();
            //then add the edge weight of the shipment FRom node TO node
            sum += g.getEdges()[schedule.get(i).getFrom().getIndexOnGraph()][schedule.get(i).getTo().getIndexOnGraph()];
        }
        //return the remaining schedule sum
        return sum;
    }
     ...
     * Sydney to vancouver <br>
     * Sydney to Manila <br>
     * Shanghai to Sydney <br>
     * and the last node in our path was sydney, since the schedule remaining has sydney to vancouver/manila, the closest
